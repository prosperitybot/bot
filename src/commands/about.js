...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const translators = await User.count({ where: { access_levels: { [Op.substring]: 'TRANSLATOR' } } });
      const administrators = await User.count({ where: { access_levels: { [Op.substring]: 'ADMINISTRATOR' } } });
      const developers = await User.count({ where: { access_levels: { [Op.substring]: 'DEVELOPER' } } });
      const owners = await User.count({ where: { access_levels: { [Op.substring]: 'OWNER' } } });
